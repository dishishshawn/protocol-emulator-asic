...
[Official template](https://github.com/TinyTapeout/ttihp-verilog-template/tree/cmos5l): CMOS5L flow. The downloaded template's tile-size comment does not list 8×4, although the competition explicitly requests it; verify the current flow supports the allocation before treating the configuration as validated.
